...
bool JT_PushMessage::take(const QDomElement &e)
{
    ...
    Jid fromJid = Jid(e1.attribute(QStringLiteral("from")));

    QDomNodeList list = e1.childNodes();
    for (int i = 0; i < list.size(); ++i) {
        QDomElement el = list.at(i).toElement();

        if (el.attribute(QStringLiteral("xmlns")) == QLatin1String("urn:xmpp:carbons:2")
            && (el.tagName() == QLatin1String("received") || el.tagName() == QLatin1String("sent"))
            && fromJid.compare(Jid(e1.attribute(QStringLiteral("to"))), false)) {
            QDomElement el1 = el.firstChildElement();
            if (el1.tagName() == QLatin1String("forwarded")
                && el1.attribute(QStringLiteral("xmlns")) == QLatin1String("urn:xmpp:forward:0")) {
                QDomElement el2 = el1.firstChildElement(QStringLiteral("message"));
                if (!el2.isNull()) {
                    forward = el2;
             ...
             && el.attribute(QStringLiteral("xmlns")) == QLatin1String("urn:xmpp:forward:0")) {
            forward = el.firstChildElement(QStringLiteral("message")); // currently only messages are supportted
